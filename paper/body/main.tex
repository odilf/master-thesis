...
For generic forces, we have to follow \cite[Lemma 4.4]{ferraroParallelIterativeMethod2025} (convergence of 1-step Jacobi-Newton): For a general smooth system $f(x) = 0$ (not necessarily a gradient system), the 1-step block Jacobi-Newton method converges locally to $x^*$ if the diagonal blocks of $Df(x^*)$ are regular and $\rho(J(x^*)) < 1$, where $J = -D^{-1}C$, $D$ = block-diagonal part of $Df$, $C$ = off-diagonal part.
...
\begin{proof}
    ...
    \]
    For a $C^2$ Lagrangian, Schwarz's theorem \tc{} gives $\mdif{21} L_d = {(\mdif{12} L_d)}^T$. Together with~\eqref{eq:force-symmetry}, this gives $\mathcal{C}^-_k = {(\mathcal{C}^+_k)}^T$. The diagonal blocks $\mathcal{D}_k$ involve only $\mdif{ii}$ derivatives, which are symmetric. Given a symmetric tridiagonal $H^f$, the proof of  Dapply verbatim with $H$ replaced by $H^f$.
\end{proof}
...
\subsubsection{A perturbation condition (rigorous)}

The second route is more conservative but does not depend on the missing lemma above. The idea is to decompose
\begin{equation}
    H^f(\mathbf{q}) = H(\mathbf{q}) + \Delta F(\mathbf{q}),
    \label{eq:hessian-matrix-forced-relative}
\end{equation}
where $H$ is the unforced Hessian~\eqref{eq:hessian-matrix} and $\Delta F$ is the block-tridiagonal force-perturbation matrix with blocks
\begin{align*}
...
\end{align*}
Note that $\Delta F = 0$ when all forces vanish, recovering $H^f = H$, so $\Delta F$ measures exactly the deviation from the unforced problem.

\begin{theorem}[Perturbation condition]\label{thm:forced-perturbation}
    Suppose the unforced Jacobi-Newton method converges at $\mathbf{q}^*$: the diagonal $D$ of $H(\mathbf{q}^*)$ is invertible and $\rho(J(\mathbf{q}^*)) < 1$. Then there exists $\varepsilon > 0$ such that, whenever $\|\Delta F(\mathbf{q}^*)\|_2 < \varepsilon$, the forced Jacobi-Newton method also converges locally to $\mathbf{q}^*$.
\end{theorem}
\begin{proof}
    Write $H^f = H + \Delta F = (D + \Delta D) - (-(L + U) - (\Delta L + \Delta U))$, where $D, L, U$ are the block-diagonal, strict block-lower, and strict block-upper parts of $H$, and similarly for $\Delta F$.

    \emph{Invertibility of the diagonal.} Each $D_k$ is invertible by hypothesis, so $\|D^{-1}\|_2 < \infty$ (block-wise). For $\|\Delta D\|_2 < \|D^{-1}\|_2^{-1}$, the Neumann series gives that $D + \Delta D$ is invertible with
    \[
        {(D + \Delta D)}^{-1} = D^{-1}\sum_{n \geq 0} (-\Delta D \, D^{-1})^n,
    \]
    and $\|{(D + \Delta D)}^{-1}\|_2 \leq \|D^{-1}\|_2 / (1 - \|D^{-1}\|_2 \|\Delta D\|_2)$.

    \emph{Continuity of the spectral radius.} The iteration matrix
    \[
        J^f = -{(D + \Delta D)}^{-1}\bigl( (L + U) + (\Delta L + \Delta U) \bigr)
    \]
    is a continuous function of $\Delta F$ at $\Delta F = 0$ (by the bound above, plus continuity of matrix products). The spectral radius $\rho$ is upper-semicontinuous on $M_n(\RR)$~\tc, so
    \[
        \limsup_{\|\Delta F\| \to 0} \rho(J^f) \leq \rho(J) < 1.
    \]
    Hence for any $\eta$ with $\rho(J) < \eta < 1$ there is $\varepsilon > 0$ such that $\|\Delta F\|_2 < \varepsilon$ implies $\rho(J^f) < \eta < 1$, which together with invertibility of $D + \Delta D$ is exactly the hypothesis of~\cite[Lemma~4.4]{ferraroParallelIterativeMethod2025}.
\end{proof}

\begin{remark}[A checkable bound]\label{rem:forced-perturbation-checkable}
    Unlike the conditions of~\autoref{thm:forced-local-implies-global}, the perturbation bound is checkable in terms of derivatives of the discrete forces $f_d^\pm$ alone---which depend on the discrete Lagrangian and the chosen discretization of the force, but not on the (unknown) solution trajectory. The constant $\varepsilon$ in the theorem is explicit but cumbersome to write down; the cleanest form is via a contractive norm. If there exists a norm $\|\cdot\|_*$ in which $\|J\|_* \leq 1 - \delta$, then $\|J^f\|_* < 1$ as soon as $\|J^f - J\|_* < \delta$, and the latter can be bounded by elementary estimates on $\|D^{-1}\Delta D\|$ and $\|D^{-1}(\Delta L + \Delta U)\|$.
\end{remark}
