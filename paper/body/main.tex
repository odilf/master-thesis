...
\section{Where the vector-space step fails}\label{sec:lie-vector-space-fails}
...
The \newterm{Lie algebra} $\fg := T_e G$ is the tangent space at the identity, forming a vector space of the same dimension as $G$.
For any other point $g \in G$, tangent directions are supplied by the differential $T_e L_g : \fg \to T_g G$ of the left-multiplication map $L_g : h \mapsto gh$, which is a isomorphism between these two vector spaces.

In the vector-space case, the Newton update has form of $q_\textrm{new} = q_k + \delta q$: you compute a correction $\delta q$ in a vector space and add it. On $G$, there's no addition, so ``compute a correction and add it'' has no meaning. What we do instead is pick a vector space (here, $\fg$), compute a correction $\delta \xi$ there, and then map it to a nearby point in $G$ via some smooth curve.

Such a map that turns an algebra direction $\delta \xi \in \fg$ into a point in $G$ near $g$ is called a \newterm{retraction} $\tau : \fg \to G$ where it has to satisfy $\tau(0) = e$ and $T_0\tau = \mathrm{Id}_\fg$.
The canonical choice is the \newterm{exponential map} $\exp : \fg \to G$, which is a local diffeomorphism at $0 \in \fg$.
Since $\exp$ maps algebra directions to group elements near $e$, left-translating by $g_k$ gives a map centred at $g_k$:
\begin{equation}
...
The correction $\delta \xi_k$ lives in the vector space $\fg$ and can be solved for by a linear system.

\begin{remark}[Other retractions]
	The exponential map is one choice of retraction, but not the only one.
	Any first-order retraction $\tau$ does the job equally well for the convergence analysis (\autoref{sec:lie-groups-convergence}).
	Namely, for matrix Lie groups, the \newterm{Cayley map} $\operatorname{cay}(\xi) = {(I - \xi/2)}^{-1}(I + \xi/2)$ is the retraction of choice in practice, being simpler to compute.
\end{remark}

\section{The algorithm on Lie algebras}\label{sec:lie-groups-algorithm}

We now linearise the residual $r_k(\mathbf{g}, g_k \exp \delta\xi) = 0$ at $\delta \xi = 0$ to obtain a linear system on $\fg$, the analogue of the vector-space Jacobi-Newton step~\eqref{eq:jacobi-newton}.
...
\end{equation}
Commonly, $T_e L_g(\eta)$ is written as $g \cdot \eta$, which looks like somewhat of an abuse of notation, but is the direct computation for the case of matrix Lie groups (both $G$ and $T_g G$ are matrices, and the differential application is, in this case, matrix multiplication).
...
\end{align*}
which pulls covectors back to the algebra. This is a (specific case of a) \newterm{pullback}. Note that since $T_e L_{g_k}$ is a diffeomorphism, both $T_e L_{g_k}$ and its transpose are isomorphism and this mapping can be done without loss of information.
...
Differentiating $r_k(\mathbf{g}, g_k \exp \delta\xi)$ in $\delta\xi$ at $0$ and applying~\eqref{eq:lie-retraction-tangent} via the chain rule gives the linear system
\begin{equation}
	\widetilde{\mathcal{D}}_k\, \delta \xi_k = -\widetilde{r}_k,
	\tag{Jacobi-Newton-G}
	\label{eq:jacobi-newton-G}
\end{equation}
where the \newterm{Lie-algebra Hessian block} $\widetilde{\mathcal{D}}_k : \fg \to \fg^*$ at node $k$ is the left-translation pullback of the second-derivative blocks of $\Ld$:
\begin{equation}
...
\end{equation}
Strictly speaking, differentiating also produces a second-order ``retraction-curvature'' term proportional to the residual $r_k$ itself; we omit it from $\widetilde{\mathcal{D}}_k$, which yields a simplified Newton step that agrees with the true Hessian at the solution (where the residual vanishes). We verify this in \autoref{sec:lie-hessian}.
...
The Hessian computation needs the second derivative of $\xi \mapsto g \exp \xi$.
In matrix-Lie-group notation, using $\exp(\xi) = I + \xi + \tfrac{1}{2} \xi^2 + O(\xi^3)$,
\begin{equation}
...
\]
matching~\eqref{eq:lie-jacobi-newton-blocks}.
This confirms that the simplified Newton step used by the algorithm employs the true Hessian diagonal block at the solution.
