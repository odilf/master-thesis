...
\begin{theorem}\label{thm:local-hessian-pd-forced}
	Given the local Hessian matrices $\mathcal{A}_k$, $\mathcal{B}_k$ as defined in \autoref{eq:hessian-terms-forced-2}, the global Hessian matrix $H^f$ is positive definite if and only if	
	\begin{itemize}
		\setlength\itemsep{0.1em}
		\item for all $k$, the local Hessian $H_k$ is positive definite, or
		\item \todo{I'm not sure about the second thingy}
...
\end{equation}
where $\Delta F(\mathbf{q})$ is the block-tridiagonal force-perturbation matrix with blocks
\begin{align*}
...
\end{align*}
Note that $\Delta F = 0$ when all forces vanish, recovering $H^f = H$.
