...
\todo{improve organization, messy rn}
...
In this setting it is not Hamilton's principle that appiles, but rather its forced analog, called the \newterm{Lagrange-D'Alembert principle}:
...
The discretization of the force is similar to the one of the Lagrangian, except that the force $f : TQ \to T^*Q$ is mapped to a \emph{pair} of discrete forces $f^\pm_d : Q \times Q \to T^*Q$. This is necessary for there to exist some ``exact discrete forces'' that recover exactly the continous forces, which we normally approximate with a quadrature rule \todo{spell out exactly how the exact discrete forces would look like}.
