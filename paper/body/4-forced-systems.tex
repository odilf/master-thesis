...
\begin{definition}[Lagrange-d'Alembert principle {\cite[Definition 4.1.1]{satomartindealmagroDiscreteMechanicsForced2020}}]
...
The discretization of the force is similar to the one of the Lagrangian, except that the force $f : TQ \to T^*Q$ is mapped to a \emph{pair} of discrete forces $f^\pm_d : Q \times Q \to T^*Q$ (where $f^-$ is fibre-preserving with respect to the first  argument, that is, $\hbox{pr}_1=f_d^-\circ \tau_Q$ and $f^+$ with the second, $\hbox{pr}_2=f_d^+\circ \tau_Q$)~\cite[\S3.2.1]{marsdenDiscreteMechanicsVariational2001}.
\todo{Esto no sería así? $\hbox{pr}_1=\pi_Q \circ f_d^-$, $\hbox{pr}_2=\pi_Q \circ f_d^+$. Sebastián}

\begin{definition}[Discrete Lagrange-d'Alembert principle {\cite[Definition 4.2.2]{satomartindealmagroDiscreteMechanicsForced2020}}]\label{def:lagrange-dalembert}
