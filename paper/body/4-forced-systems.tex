...
Lagrangian systems can be extended to \newterm{Lagrangian forced systems}, defined with a \newterm{Lagrangian external force} which is usually defined as a map $f^L : TQ \to T^* Q$~\cite[section 4.1.1]{satomartindealmagroDiscreteMechanicsForced2020} that is fibre-preserving. This system is now not determined only by the Lagrangian, but by the discrete Lagrangian and the external force.

In this setting it is not Hamilton's principle that applies, but rather its forced analog, called the \newterm{Lagrange-D'Alembert principle}:
...
Of course, the question is how to make ``large enough'' precise. An approach that seems like it would work is to ask whether $\lambda_{\min}(\Sym(H^f)) > \|\Skew(H^f)\|_2$ (this confines the eigenvalues of $H^f$ to a $45^\circ$ sector in the right half-plane and is another natural generalization of $M \succ 0$ to a non-symmetric setting). This is provable in the scalar $n = 1$, $N = 3$ case (see \texttt{notes/forced-systems-convergence.tex}, Appendix~A) but open in the general block case. What we can prove is a stronger condition \emph{per edge}: ask the symmetric part of each diagonal sub-block of $H_k^f$ to strictly dominate the corresponding cross sub-block in spectral norm. This way, we arrive at the following theorem.
...
\Cref{ex:parachutist} shows a practical application of the algorithm to find trajectories for a parachutist in turbulent wind conditions.
...
\tocless\subsection{Charged particle in a uniform magnetic field}\label{ex:lorentz}
...
% \tocless\subsection{Effect of force discretization on convergence}\label{ex:discretization}
...
\todo{I don't know if ``turbulent'' is technically correct, since I think it technically means non-laminar flow?}

A parachutist wants to jump from a helicopter at some position $(x_0, y_0)$ and reach a target point in the ground $(x^T, 0)$ at some time $T$ by letting themselves be carried by the wind, which is given as some vector field $W(x, y) = (w_x, w_y)$.

This problem can be modelled with the Lagrangian
\[
    L(x, y, \dot{x}, \dot{y})=\frac{m}{2}(\dot{x}^2+\dot{y}^2)-m g y
\]
where $g$ is the gravitational acceleration. Furthermore, the parachutist feels some drag, which is modelled as an external force:
\begin{align*}
	(F_x, F_y)=(-k(\dot{x}-w_x(x, y)), -k(\dot{y}-w_y(x, y))), 
\end{align*}
where $k$ is the parachute drag coefficient.

The problem can be discretized with the midpoint rule, for instance. The only additional data needed are the length of the path $N$, which then gives us the timestep $h = \frac{T}{N}$.

Finally, we can recover what the initial 

    \textcolor{blue}{David
    Parachutist example
    Problem: Calculate the jump velocity and direction to reach a desired target point at a given time $T$.
    The problem is given by: 
    $$L(x, y, \dot{x}, \dot{y})=\frac{m}{2}(\dot{x}^2+\dot{y}^2)-mgy$$
and external force
\begin{align*}
(F_x, F_y)=(-k(\dot{x}-w_x(x, y)), -k(\dot{x}-w_y(x, y))), 
\end{align*}
where $(w_x, w_y)$    is the wind velocity in both components (we assume that it is time-independent). The parameter $k$ is the parachute drag. 
coefficient 
Now, given a point $(x_0, y_0)$ (where $y_0$ is the aircraft attitude) and $(x_T, 0)$ is the target point, we use the parallel method to find a trajectory. Here $T=nh$. 
Finally, the initial velocity  is 
\[
(\frac{x_{1}-x_0}{h}, \frac{y_1-y_0}{h})
\]
    }
