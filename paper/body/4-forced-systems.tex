...
Lagrangian systems can be extended to \newterm{Lagrangian forced systems}, defined with a \newterm{Lagrangian external force} which is usually a map $f^L : TQ \to T^* Q$~\cite[section 4.1.1]{satomartindealmagroDiscreteMechanicsForced2020} that is fibre-preserving. This system is now not determined only by the Lagrangian, but by the discrete Lagrangian and the external force.
...
Unlike Hamilton's principle, this is \emph{not} the extremization of any action: forces are generally non-conservative \todo{this is mentioned before I clarify later, but maybe it's ok}, so no modified action exists. Since the condition must hold for each $\delta q_k$ independently, it is equivalent to the following \newterm{discrete forced Euler-Lagrange equations} holding at each inner point $k$:
...
For generic forces, the relevant criterion is given in~\cite[Lemma 4.4]{ferraroParallelIterativeMethod2025} which we restate here:
