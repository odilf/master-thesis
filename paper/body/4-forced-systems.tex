...
Lagrangian systems can be extended to \newterm{Lagrangian forced systems}, defined with a \newterm{Lagrangian external force} which is usually defined as a map $f^L : TQ \to T^* Q$~\cite[section 4.1.1]{satomartindealmagroDiscreteMechanicsForced2020} that is fibre-preserving. In this setting it is not Hamilton's principle that applies, but rather its forced analog, called the \newterm{Lagrange-D'Alembert principle}:
\begin{definition}[Lagrange-D'Alembert principle {\cite[Definition 4.1.1]{satomartindealmagroDiscreteMechanicsForced2020}}]
	A path $q(t) : [0, T] \to Q \in C^2([0, T], Q)$ is the physical trajectory of the forced Largangian system defined by $L: TQ \to \RR$ and force $f^L : TQ \to T^*Q$ if and only if
	\[
...
The discretization of the force is similar to the one of the Lagrangian, except that the force $f : TQ \to T^*Q$ is mapped to a \emph{pair} of discrete forces $f^\pm_d : Q \times Q \to T^*Q$ (where $f^-$ is fibre-preserving with respect to the first  argument, that is, $\hbox{pr}_1=f_d^-\circ \tau_Q$ and $f^+$ with the second, $\hbox{pr}_2=f_d^+\circ \tau_Q$). See \cite{marsdenDiscreteMechanicsVariational2001} %This is necessary for there to exist some ``exact discrete forces'' that recover exactly the continous forces, which we normally approximate with a quadrature rule \todo{spell out exactly how the exact discrete forces would look like}.

\begin{definition}[Discrete Lagrange-D'Alembert principle {\cite[Definition 4.2.2]{satomartindealmagroDiscreteMechanicsForced2020}}]\label{def:lagrange-dalambert}
...
For generic forces, the relevant criterion that can be applied is given in~\cite[Lemma 4.4]{ferraroParallelIterativeMethod2025} which we restate here:
...
\begin{align*}
	H^f_{ij} (\mathbf{q}) & = \mdif*{\hat{R}(\mathbf{q})}                                                                                                                            \\ & = \mdif{q_j} \left(
	\mdif{2} L_d (q_{k-1}, q_k) + f_d^+ (q_{k-1}, q_k) + \mdif{1} L_d (q_k, q_{k+1}) + f_d^- (q_k, q_{k+1}) = 0
	\right)                                                                                                                                                                          \\
...
The following theorem states in a rigourous and quantitative manner the intuitive idea that, if the algorithm already converges without forces, then it still converges when you add forces as long as the forces are ``weak enough'', which manifest as the derivatives $\mdif{i} f_d^\pm$ that appear in the Jacobian being small ``enough''.
...
\begin{align*}
	...
	f_d^-(q_k, q_{k+1}) & = -\frac{\gamma}{2}(q_{k+1}-q_k).
\end{align*}
The ``blocks'' of the forced Hessian are just scalars, evaluating to
\begin{alignat*}{2}
...
Assuming the unforced oscillator converges at $\mathbf{q}^*$ for the chosen step size $h$ (a condition on $h$ and $\omega$ alone), Theorem~\ref{thm:forced-perturbation} and Lemma~\ref{lem:forced-perturbation-explicit} guarantee that the forced method also converges whenever
\[
