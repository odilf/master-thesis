...
Such a map is called a \newterm{retraction} $\tau : \fg \to G$ which has to satisfy $\tau(0) = e$ and $T_0\tau = \mathrm{Id}_\fg$. \todo{maybe mention retractions can have other properties}
...
\end{equation}
Commonly, $T_e L_g(\eta)$ is written as $g \cdot \eta$, which looks like somewhat of an abuse of notation, but is the direct computation for the case of matrix Lie groups (since in that case elements of $G$ are matrices and of $T_g G$, vectors; where the differential application is just direct matrix multiplication).
...
\todo{This section is a stub. }


\tocless\subsection{Free particle on $G$}\label{sec:lie-example-free}
...
\tocless\subsection{Rigid body on $SO(3)$}\label{sec:lie-example-rigid-body}
