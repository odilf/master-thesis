...
This is enough to implement the algorithm. The source code used for the following examples is available in the repository \todo{Add link}.
...
A more accurate way of modeling this situation is by using $SE(4)$, the group of rigidbody rotations, which automatically handles this complexity in its manifold geometry.

Namely, for a position $x$ and orientation $R$, the potential and kinetic energies are:
\begin{align*}
	V(R, x) &= -\frac{\mu_j m_i}{\| x + R,r_i^b - x_j\|} \\
	T(R, x, \dot{R}, \dot{x}) &= 
\end{align*}
% jnp.dot(v_rot, self.inertia * v_rot) / 2 + self.mass / 2 * jnp.dot(v_trans, v_trans)
where


\todo{Put pictures in.}

\tocless\subsection{Zermelo fuel navigation in $SE(3)$}
\todo{Fill in}
