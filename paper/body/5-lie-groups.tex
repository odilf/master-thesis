...
\]
In this setting, taking the derivative with $g_{k\pm1}$ fixed entails going from $L_d^\pm : G \to \RR$ to $T_{\overline{g_k}} G \to T_{L_d^\pm (\overline{g_k})} \RR$, and using the fact that every tangent bundle of $\RR^n$ is isomorphic to $\RR^n$, this becomes a sum in $T_{\overline{g_k}}^* G$. This forms a vector space so this step is well-defined.
