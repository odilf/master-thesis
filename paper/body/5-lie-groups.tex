...
\todo{I think this is a trivialization, maybe I should mention that.}
...
\end{equation}
Commonly, $T_e L_g(\eta)$ is written as $g \cdot \eta$, which looks like somewhat of an abuse of notation, but corresponds to the direct computation for the case of matrix Lie groups (since in that case elements of $G$ are matrices and of $T_g G$, vectors; where the differential application is just direct matrix multiplication).

The next step is to write the residual in the Lie-algebra.
As it stands, the residual $r_k(\mathbf{g}, g_k) \in T_{g_k}^* G$ is a covector at $g_k$, but we need it as a covector on the identity for it to be an element of $\fg$ and be able to set up a linear system there.
The main tool to do this is taking the \newterm{transpose} of $T_e L_{g_k} : \fg \to T_{g_k} G$:
\begin{align*}
	                          & {(T_e L_{g_k})}^* : T_{g_k}^* G \to \fg^*                                                    \\
	\textrm{defined as} \quad & {(T_e L_{g_k})}^* \alpha = \eta \mapsto \alpha (T_e L_{g_k} (\eta)) \quad \textrm{for } \alpha \in T_{g_k}^*
\end{align*}
...
\end{equation}
(it will be shown later that this is in fact the diagonal blocks of the Lie-algebra Hessian, as in the vector-space case).
...
The strategy consists of lifting the discrete action to a function on the vector space $\fg^{N-1}$, compute its gradient and Hessian there, and read off the convergence criterion.
...
\begin{remark}[Connection to the discrete Euler-Poincaré equations]\label{rmk:lie-DEP}
	Many mechanical systems on Lie groups are naturally \newterm{left-invariant}, meaning that $\Ld(g_k, g_{k+1}) = \Ld(h g_k, h g_{k+1})$ for all $h \in G$. If this is the case, the Lagrangian factors through the quotient $f_k = g_k^{-1} g_{k+1}$, resulting in a \newterm{reduced Lagrangian} $\ell : G \to \RR$ with $\ell(f_k) = \Ld(g_k, g_{k+1})$.
...
\begin{enumerate}
	...
	\item the pullbacked differential, ${(T_e L_g)}^* T_g$
	\item the group action
	\item a retraction
\end{enumerate}
...
The only thing missing is a way to calculate the second derivative, but this can actually be calculated with no additional knowledge, given that we're representing $\fg$ and $\fg^*$ as vectors. Namely, we differentiate directly the residual as a function of the algebra, by taking the Jacobian of $\widetilde{r_k}(\delta \xi_k) : \fg \to \fg^*$. Since we represent $\fg$ and $\fg^*$ as $\RR^n$, this can be done using any autodifferentiation library.

This is enough to implement the algorithm, although in practice we also ask for a way to construct a tangent ``velocity'' vector as \verb|tangent(q0, q1, h)| which we use to be able to define the Lagrangian in the continous way and discretize it automatically.
...
A more accurate way of modeling this situation is by using $SE(3)$, the group of rigidbody motions, which automatically handles this complexity in its manifold geometry. Namely, using $SE(3) \cong SO(3) \times \RR^3$, for a position $x$ and orientation $R$ the potential and kinetic energies are:
\begin{align*}
	V(R, x) &= -\frac{\mu_j m_i}{\| x + R\, r_i^b - x_j\|} \\
	T(R, x, \omega, v) &= \frac{\omega^T I \omega}{2} + \frac{m \|v\|}{2}
\end{align*}
where $r_i$ and $m_i$ are the position and mass of the $i$th body point, $\mu_j$ and $x_j$ are the mass and position of the $j$th celestiall object, and $\omega$ and $v$ are the angular and translational velocity. The Lagrangian for a system defined by potential and kinetic energy is $L = V - T$. We discretized it using midpoint rule and plotted the trajectory of a prims-like satellite for some chosen conditions with interesting behavior (see the source code for details), shown in \cref{fig:satellite}.
...
Lagrangian formulations can also be used to find optimal solutions to certain problems. In~\cite[Example 5.2]{ferraroParallelIterativeMethod2025} they formulate a fuel-optimal navigation problem, given a wind field. We can extend it to $SE(3)$ by making the system take into account the orientation of the ship, which is in fact a spaceship that can reorient itself in three dimensions.

Specifically, we model a prism-like ship with one long axis and two short ones, such that if the long axis is perpendicular to the flow the effect is much greater than if it is parallel.

\[ L(R, x, \omega, v) = \frac{\omega^T I \omega}{2} + \frac{{(R(v - W(x)))}^T D R(v - W(x))}{2} \]

where

\[ W(x) = \]

\todo{Write out lagrangian.}
...
\begin{figure}
     ...
     \end{subfigure}
    \caption{Zermelo fuel optimization problem in $SE(3)$. Bottow red arrows show flow field, upper arrows show how the flow affects the rigidbody in the given position.}\label{fig:zermelo}
\end{figure}
