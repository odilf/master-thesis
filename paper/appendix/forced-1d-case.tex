...
\begin{lemma}[Scalar local condition]
...
\begin{proof}
	...
	\begin{equation}
		...
		\label{eq:hyp-scalar}
	\end{equation}
	The right-hand side is a norm and the left-hand side is greater than it, so it is positive too. Before squaring, we record a bound on $|a|$ that we will need below: dropping $s^2$ under the root in~\eqref{eq:hyp-scalar} only decreases the right-hand side, so $\sqrt{\left(\tfrac{d_1-d_2}{2}\right)^2 + s^2} \geq \tfrac{|d_1-d_2|}{2}$ and therefore
	\begin{equation}
...
A few computer simulations indicate that this condition is sufficient, and in fact it seems to be considerably stricter than necessary.